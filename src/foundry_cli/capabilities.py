...
_IMPLEMENTED_EVIDENCE: dict[str, str] = {
    ...
    "send_ai_fde_message": (
        ...
        "itemsToWrite minimal-delta behavior contract-verified "
        "against a second live deployment"
    ),
    "get_ai_fde_thread_agent_state": (
        "internal ai-fde GraphQL gateway (UI capture 2026-09-04, HTTP 200): "
    ...
    "run_ai_fde_agent_loop": (
        "internal language-model-service + ai-fde + captured tool contracts "
        "(UI capture via CDP; LLM call and thread delta writes "
        "live-verified against a second deployment): CLI-side "
        "reimplementation of the AI FDE client-side agent loop — PUT "
        "/language-model-service/api/llm/v3/completion/{model}/"
        "streamCompletionChunk (JSON-array event stream; attribution user "
        "== the bearer token itself; sessionId == threadId), client-side "
        "tool execution, tool-usage/assistant-message write-back via PUT "
        "/ai-fde/api/threads/{id}/update. Full 72-tool captured catalog "
        "registered verbatim; mined mode->tool-set system (base 8 tools "
        "by default, change_mode swaps to the captured mode set, "
        "--all-tools exposes everything). 25 live executors "
        "(ontology_sql_query, evals reads/run, execute_action "
        "validate-then-apply, ontology bulk loads + associated-type "
        "GraphQL reads, object-set loads, function specs, stemma repo "
        "reads, pull-request load, container git-status + approval-gated "
        "terminal commands, state tools); the other 47 (schedules, "
        "ci_checks, logic family, documentation, ...) fail closed with "
        "typed UnverifiedContract results. Write tools are "
        "approval-gated"
    ),
    ...
    "create_python_transforms_code_repository": (
        ...
        "the captured contract; foundry contract-verified "
...
_U3_TEST_REFERENCES: dict[str, str] = {
    ...
    "run_ai_fde_agent_loop": "tests/test_services/test_ai_fde_loop.py;tests/test_commands/test_ai_fde.py",
}
...
# Write-capable capabilities whose ids the prefix heuristic cannot catch
# (the agent loop is read-mostly but executes approval-gated write tools).
_WRITE_RISK_IDS: frozenset[str] = frozenset({"run_ai_fde_agent_loop"})
...
def registered_command_paths() -> frozenset[str]:
    ...
    fully assembled. This is the same surface `foundry agent-manifest` emits, so a
    ...
    for group, capability_id, command, service, evidence in (
        ...
        *_WORKFLOW_ROWS,
        ...
        if capability_id.startswith(
            ...
            mutation_risk = "write"
        if capability_id in _WRITE_RISK_IDS:
            mutation_risk = "write"
...
def __getattr__(name: str) -> Any:
    # PEP 562: keep `from foundry_cli.capabilities import CAPABILITIES` working without
