...
class OntologyService(BaseService):
    ...
    def get_ontology(self, ontology_rid: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to get ontology {ontology_rid}: {self._describe_error(e)}"
            )
    ...
    def get_ontology_rid(self) -> Dict[str, Any]:
        ...
        if len(ontologies) > 1:
            ...
            raise RuntimeError(
                ...
                "resolved unambiguously. Pick one with 'foundry ontology list': "
                f"{choices}"
            )
        return ontologies[0]
...
class ObjectTypeService(BaseService):
    ...
    def list_object_types(self, ontology_rid: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to list object types: {self._describe_error(e)}"
            )
    ...
    def get_object_type(self, ontology_rid: str, object_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to get object type {object_type}: {self._describe_error(e)}"
            ) from e
            ...
            return (
                {
                    "status": "not-verified",
                    "detail": (
                        f"read-back via bulkLoadEntities failed: {self._describe_error(e)}"
                    ),
                },
                None,
            )
    ...
    def delete_object_type(
        ...
        if "." not in object_type_id:
            raise RuntimeError(
                ...
                "returned by 'foundry ontology object-type-upsert' and visible "
    ...
    def _discover_object_type_namespace(self, client: Any, ontology_rid: str) -> str:
        ...
        probe_request = self._build_object_type_modification_request(
            ...
            api_name="FoundryNamespaceProbe",
            display_name="foundry namespace check",
            primary_key="id",
            backing_dataset="ri.foundry.main.dataset.foundry-namespace-probe",
            description=None,
        )
        status, parsed, raw = client.conjure(
            "POST",
    ...
    def _verify_object_type_present(
        ...
        try:
            ...
            return {
                "status": "not-verified",
                "detail": (
                    f"read-back via SDK ontologies ObjectType.get failed: {self._describe_error(e)}"
                ),
            }
        return {
            "status": "verified",
    ...
    def delete_link_type(
        ...
        if "." not in link_type_id:
            raise RuntimeError(
                ...
                "returned by 'foundry ontology link-type-upsert' and visible "
                "in the Foundry ontology manager."
            )
        client = _internal_client(self)
        modification_request: Dict[str, Any] = {
    ...
    def get_link_type(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to get link type {link_type}: {self._describe_error(e)}"
            )
...
class OntologyObjectService(BaseService):
    ...
    def get_object(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to get object {primary_key}: {self._describe_error(e)}"
            )
    ...
    def aggregate_objects(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to aggregate objects: {self._describe_error(e)}"
            )
    ...
    def list_linked_objects(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to list linked objects: {self._describe_error(e)}"
            )
    ...
    def prepare_object_upsert(
        ...
        if primary_key_property in parameters and str(
            parameters[primary_key_property]
...
class ActionService(BaseService):
    ...
    def apply_action(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to apply action {action_type}: {self._describe_error(e)}"
            )
    ...
    def validate_action(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to validate action {action_type}: {self._describe_error(e)}"
            )
    ...
    def apply_batch_actions(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to apply batch actions: {self._describe_error(e)}"
            )
    ...
    def apply_action_with_overrides(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to apply action {action_type} with overrides: {self._describe_error(e)}"
            )
    ...
    def apply_batch_with_overrides(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to apply batch actions with overrides: {self._describe_error(e)}"
            )
    ...
    def get_action_type(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to get action type {action_type}: {self._describe_error(e)}"
            )
