...
"contents": []}``). PARTIALLY-DERIVED: the only captured ``/update`` call
wrote every item in the thread; the delta behavior of ``itemsToWrite`` was
never observed. This service sends a minimal delta (``itemIdsInOrder`` =
existing order + new id, ``itemsToWrite`` = just the new item), inferred
from the field names — if the server requires full-item rewrites this will
fail loudly with a typed error rather than silently corrupt the thread.
...
class AiFdeService(BaseService):
    ...
    def update_settings(self, settings_modification: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    @staticmethod
    def build_user_message_item(text: str, context_item_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    def send_user_message(self, thread_id: str, text: str) -> Dict[str, Any]:
        ...
        NOTE (partially-derived): the delta behavior of ``itemsToWrite``
        was never captured — only a full-thread write. This sends a minimal
        delta (new item only); see the module docstring.
        """
        metadata = self.get_thread_metadata(thread_id)
