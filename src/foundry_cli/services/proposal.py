...
- ``code-pr`` delegates to :class:`~foundry_cli.services.repository.RepositoryService`
  (internal ``stemma-pull-request`` API): list, get, create, comment, close.
- ``global-proposal`` delegates to
  :class:`~foundry_cli.services.global_branching.GlobalProposalService` (internal
