"""Shell completion commands for foundry CLI."""
...
console = Console()
app = typer.Typer(help="Manage shell completions for foundry CLI")
...
def install(
    ...
    """Install shell completions for foundry CLI.
    ...
    Examples:
        foundry completion install
        foundry completion install --shell bash
        foundry completion install --shell zsh --path ~/.config/custom/completions
    """
...
def generate_bash_completion() -> str:
    ...
    return """# foundry bash completion script
# Generated by foundry completion install

_foundry_completion() {
    local IFS=$'\\n'
    local response

    response=$(env COMP_WORDS="${COMP_WORDS[*]}" COMP_CWORD=$COMP_CWORD _FOUNDRY_COMPLETE=bash_complete foundry)
...
_foundry_completion_setup() {
    complete -o nosort -F _foundry_completion foundry
}
...
_foundry() {
    ...
    local -a response
    (( ! $+commands[foundry] )) && return 1

    response=("${(@f)$(env COMP_WORDS="${words[*]}" COMP_CWORD=$((CURRENT-1)) _FOUNDRY_COMPLETE=zsh_complete foundry)}")
...
def generate_fish_completion() -> str:
    ...
    return """# foundry fish completion script
# Generated by foundry completion install

function _foundry_completion
    set -l response (env COMP_WORDS=(commandline -cp) COMP_CWORD=(commandline -t) _FOUNDRY_COMPLETE=fish_complete foundry)
...
complete -c foundry -f -a "(_foundry_completion)"
"""
...
def get_default_completion_path(shell: str) -> Path:
    ...
    if shell == "bash":
        ...
        paths = [
            home / ".local" / "share" / "bash-completion" / "completions" / "foundry",
            home / ".bash_completion.d" / "foundry",
            Path("/etc/bash_completion.d/foundry"),
        ]
        ...
        return home / ".zfunc" / "_foundry"

    elif shell == "fish":
        # Fish completion path
        return home / ".config" / "fish" / "completions" / "foundry.fish"
...
def uninstall(
    ...
    """Uninstall shell completions for foundry CLI."""
    # Auto-detect shell if not specified
    if shell is None:
        shell = get_shell_from_env()
        if shell is None:
