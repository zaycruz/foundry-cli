"""Shell completion commands for pfoundry CLI."""
...
console = Console()
app = typer.Typer(help="Manage shell completions for pfoundry CLI")
...
def install(
    ...
    """Install shell completions for pfoundry CLI.
    ...
    Examples:
        pfoundry completion install
        pfoundry completion install --shell bash
        pfoundry completion install --shell zsh --path ~/.config/custom/completions
    """
...
def generate_bash_completion() -> str:
    ...
    return """# pfoundry bash completion script
# Generated by pfoundry completion install

_pfoundry_completion() {
    local IFS=$'\\n'
    local response

    response=$(env COMP_WORDS="${COMP_WORDS[*]}" COMP_CWORD=$COMP_CWORD _PFOUNDRY_COMPLETE=bash_complete pfoundry)
...
_pfoundry_completion_setup() {
    complete -o nosort -F _pfoundry_completion pfoundry
}
...
def generate_zsh_completion() -> str:
    ...
    return """#compdef pfoundry
# pfoundry zsh completion script
# Generated by pfoundry completion install
...
_pfoundry() {
    ...
    local -a response
    (( ! $+commands[pfoundry] )) && return 1

    response=("${(@f)$(env COMP_WORDS="${words[*]}" COMP_CWORD=$((CURRENT-1)) _PFOUNDRY_COMPLETE=zsh_complete pfoundry)}")
...
def generate_fish_completion() -> str:
    ...
    return """# pfoundry fish completion script
# Generated by pfoundry completion install

function _pfoundry_completion
    set -l response (env COMP_WORDS=(commandline -cp) COMP_CWORD=(commandline -t) _PFOUNDRY_COMPLETE=fish_complete pfoundry)
...
complete -c pfoundry -f -a "(_pfoundry_completion)"
"""
...
def get_default_completion_path(shell: str) -> Path:
    ...
    if shell == "bash":
        ...
        paths = [
            home / ".local" / "share" / "bash-completion" / "completions" / "pfoundry",
            home / ".bash_completion.d" / "pfoundry",
            Path("/etc/bash_completion.d/pfoundry"),
        ]
        ...
        return home / ".zfunc" / "_pfoundry"

    elif shell == "fish":
        # Fish completion path
        return home / ".config" / "fish" / "completions" / "pfoundry.fish"
...
def uninstall(
    ...
    """Uninstall shell completions for pfoundry CLI."""
    # Auto-detect shell if not specified
    if shell is None:
        shell = get_shell_from_env()
        if shell is None:
