...
def render_markdown(manifest: Optional[Mapping[str, Any]] = None) -> str:
    """Render the parity scorecard as a deterministic Markdown report.

    This is the human-facing form of the capability manifest. It is generated,
    not hand-written -- `tests/test_capabilities_doc.py` regenerates it and
    fails if the committed CAPABILITIES.md has drifted, so the parity claim can
    never go stale. There is no runtime `pltr capabilities` command; an agent
    discovers commands with `pltr agent-manifest`, and this report is for a
    human evaluating MCP parity and roadmap.
    """
    if manifest is None:
    ...
    caps = sorted(
        ...
        key=lambda c: (c["group"], c["capability_id"]),
    )
    ...
    lines: list[str] = [
        ...
        f"the {catalog['tool_count']} tools and {catalog['workflow_count']} workflow is",
        ...
        f"([source]({catalog['source_url']}), retrieved {catalog['retrieved_on']})",
        f"- **Totals:** {counts['implemented']} implemented, "
    ...
    for group in sorted(groups):
        b = groups[group]
        lines.append(
            f"| {group} | {b['total']} | {b['implemented']} | {b['planned']} "
            f"| {b['blocked']} | {b['unsupported']} |"
        )
    ...
    for cap in caps:
        status = _STATUS_LABEL.get(cap["status"], cap["status"])
        command = f"`pltr {cap['command']}`" if cap["status"] == "implemented" else "—"
        note = (cap.get("blocked_reason") or "").replace("\n", " ").strip()
        lines.append(f"| `{cap['capability_id']}` | {status} | {command} | {note} |")
...
if __name__ == "__main__":  # pragma: no cover - regeneration entrypoint
    ...
    target = pathlib.Path(__file__).resolve().parents[2] / "CAPABILITIES.md"
