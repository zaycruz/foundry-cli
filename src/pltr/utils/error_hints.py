...
ONTOLOGY_GET_NOT_FOUND_HINT = (
    "Do not probe name variants with repeated gets: `pltr ontology resolve` "
    "(--kind object-type|action-type with exactly one of --api-name/--rid) "
    "maps API name <-> RID <-> internal IDs and checks existence in one "
    "call, or `pltr ontology object-type-list` enumerates object types."
)
...
OBJECT_TYPE_UPSERT_HINT = (
    ...
    "`pltr ontology object-type-guarded-upsert`."
)
...
ACTION_TYPE_UPSERT_HINT = (
    ...
    "for object types, `pltr ontology object-type-guarded-upsert` preflights "
