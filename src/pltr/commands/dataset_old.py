...
# Note: list command removed because foundry-platform-sdk doesn't support dataset listing
...
def upload_file(
    ...
    transaction_rid: Optional[str] = typer.Option(
        None, "--transaction", "-t", help="Transaction RID"
    ),
):
    ...
    try:
        ...
        formatter.print_success(
            f"Successfully uploaded {file_path_obj.name} to dataset {dataset_rid}"
        )
        formatter.print_info(f"Branch: {result['branch']}")
        formatter.print_info(
            f"File size: {formatter._format_file_size(result['size_bytes'])}"
        )
...
def download_file(
    ...
    try:
        ...
        if progress:
            tracker = FileProgressTracker()
            with tracker.track_download(
                output_path, description=f"Downloading {Path(file_path).name}"
            ):
            ...
            with SpinnerProgressTracker().track_spinner(
                f"Downloading {Path(file_path).name}..."
            ):
                result = service.download_file(
                    dataset_rid=dataset_rid,
                    file_path=file_path,
...
def list_files(
    ...
    try:
        ...
        with SpinnerProgressTracker().track_spinner(
            f"Listing files in dataset {dataset_rid}..."
        ):
            files = service.list_files(dataset_rid=dataset_rid, branch=branch)

        if not files:
            formatter.print_info(
                f"No files found in dataset {dataset_rid} on branch {branch}"
            )
            return
...
def delete_dataset(
    ...
    if not force:
        confirm = typer.confirm(
            f"Are you sure you want to delete dataset {dataset_rid}?"
        )
        if not confirm:
    ...
    try:
        ...
        with SpinnerProgressTracker().track_spinner(
            f"Deleting dataset {dataset_rid}..."
        ):
...
def list_branches(
    ...
    try:
        ...
        with SpinnerProgressTracker().track_spinner(
            f"Listing branches for dataset {dataset_rid}..."
        ):
        ...
        if output:
            ...
            formatter.print_info(
                f"Found {len(branches)} branch(es) for dataset {dataset_rid}"
            )
...
def create_branch(
    ...
    try:
        ...
        with SpinnerProgressTracker().track_spinner(
            f"Creating branch '{branch_name}' for dataset {dataset_rid}..."
        ):
