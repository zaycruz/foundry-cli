...
TOOL_MANIFEST_SCHEMA_VERSION = "pltr-cli-tool-manifest-v1"
...
def _supported_option_names(parameter: click.Option) -> list[str]:
    return [
        ...
        if name.startswith("--") or (len(name) == 2 and name.startswith("-") and name[1].isalpha())
    ]
...
def _risk(path: tuple[str, ...], command: click.Command) -> str:
    if any(
        ...
        return "unknown"
    if path[-1].rsplit("-", 1)[-1] in READ_ONLY_VERBS or "read-only" in (command.help or "").casefold():
        return "read"
    return "unknown"
...
def _parameter_manifest(
    ...
    *,
    ...
    if isinstance(parameter, click.Option):
        ...
        if parameter.is_flag:
            if not isinstance(parameter.flag_value, bool) or (
                parameter.default is not None and not isinstance(parameter.default, bool)
            ):
                return None
    ...
    if isinstance(parameter.type, click.Choice) and all(
        isinstance(choice, str) and choice.strip() == choice and choice for choice in parameter.type.choices
    ):
    ...
    return {
        ...
        "default": parameter.default if isinstance(parameter, click.Option) and parameter.is_flag else None,
        "enum": enum,
...
def _command_manifest(path: str, command: click.Command) -> dict[str, Any] | None:
    ...
    for positional_index, parameter in enumerate(
        parameter for parameter in command.params if isinstance(parameter, click.Argument)
    ):
        record = _parameter_manifest(parameter, positional_index=positional_index)
        if record is None:
            return None
        parameters.append(record)
    positional_parameters = [parameter for parameter in parameters if parameter["mapping"]["kind"] == "positional"]
    ...
    for option in command.params:
        ...
        if "--profile" in long_names:
            profile = {
                "kind": "option",
                "argv": "--profile",
                "aliases": [name for name in _supported_option_names(option) if name != "--profile"],
            }
            continue
