...
from ..auth.base import ProfileNotFoundError, MissingCredentialsError, InvalidCredentialsError
...
def verify(
    ctx: typer.Context,
    profile: Optional[str] = typer.Option(None, "--profile", "-p", help="Profile to verify"),
):
    ...
    try:
        ...
        if not host:
            console.print(f"[red]Error:[/red] No host URL configured for profile '{active_profile}'")
            raise typer.Exit(1)
        ...
        if auth_type == "token":
            ...
            if not token:
                console.print(f"[red]Error:[/red] No token configured for profile '{active_profile}'")
                raise typer.Exit(1)
            ...
            if not client_id or not client_secret:
                console.print(f"[red]Error:[/red] OAuth credentials incomplete for profile '{active_profile}'")
                raise typer.Exit(1)
            ...
            if token_response.status_code == 200:
                ...
                console.print(f"[red]Error:[/red] Failed to get OAuth token: {token_response.text}")
                raise typer.Exit(1)
        ...
        if response.status_code == 200:
            ...
            if 'organization' in user_info:
                table.add_row("Organization", user_info['organization'].get('rid', 'N/A'))
            
            # Add groups if available
            if 'groups' in user_info and user_info['groups']:
                groups_list = ', '.join(g.get('name', 'N/A') for g in user_info['groups'][:3])
            ...
            console.print(table)
            console.print()
            console.print(f"[green]Profile '{active_profile}' is properly configured![/green]")
            
        elif response.status_code == 401:
            console.print(f"[red]✗[/red] Authentication failed: Invalid credentials")
            console.print(f"Please check your token/credentials for profile '{active_profile}'")
            raise typer.Exit(1)
        elif response.status_code == 403:
            console.print(f"[red]✗[/red] Authentication failed: Access forbidden")
            console.print("Your credentials are valid but you don't have access to this endpoint")
            raise typer.Exit(1)
        else:
        ...
        console.print(f"[red]✗[/red] Connection failed: Unable to reach the Foundry host")
