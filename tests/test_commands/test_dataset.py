...
def test_list_datasets_profile_not_found(mock_dataset_service):
    ...
    mock_dataset_service.list_datasets.side_effect = ProfileNotFoundError(
        "Profile not found"
    )
...
def test_list_datasets_missing_credentials(mock_dataset_service):
    ...
    mock_dataset_service.list_datasets.side_effect = MissingCredentialsError(
        "Missing token"
    )
...
def test_upload_file_success(mock_dataset_service):
    ...
    try:
        result = runner.invoke(
            app, ["upload", "ri.foundry.main.dataset.test", temp_path]
        )
...
def test_upload_file_not_found(mock_dataset_service):
    ...
    result = runner.invoke(
        app, ["upload", "ri.foundry.main.dataset.test", "/nonexistent/file.csv"]
    )
...
def test_upload_file_directory(mock_dataset_service):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        result = runner.invoke(
            app, ["upload", "ri.foundry.main.dataset.test", temp_dir]
        )
...
def test_upload_file_no_progress(mock_dataset_service):
    ...
    try:
        result = runner.invoke(
            app, ["upload", "ri.foundry.main.dataset.test", temp_path, "--no-progress"]
        )
...
def test_download_file_default_output(mock_dataset_service):
    ...
    result = runner.invoke(
        app, ["download", "ri.foundry.main.dataset.test", "data.csv"]
    )
...
def test_list_files_with_branch(mock_dataset_service, sample_files):
    ...
    result = runner.invoke(
        app, ["files", "ri.foundry.main.dataset.test", "--branch", "feature"]
    )

    assert result.exit_code == 0
    mock_dataset_service.list_files.assert_called_once_with(
        dataset_rid="ri.foundry.main.dataset.test", branch="feature"
    )
...
def test_delete_dataset_success(mock_dataset_service):
    ...
    mock_dataset_service.delete_dataset.assert_called_once_with(
        "ri.foundry.main.dataset.test"
    )
...
def test_list_branches_success(mock_dataset_service, sample_branches):
    ...
    assert result.exit_code == 0
    mock_dataset_service.get_branches.assert_called_once_with(
        "ri.foundry.main.dataset.test"
    )
...
def test_create_branch_success(mock_dataset_service):
    ...
    result = runner.invoke(
        app, ["create-branch", "ri.foundry.main.dataset.test", "new-branch"]
    )
