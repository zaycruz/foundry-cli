...
def test_manifest_contains_registered_commands_and_flags() -> None:
    ...
    parameters = {parameter["name"]: parameter for parameter in object_type_list["parameters"]}
    ...
    assert "dataset_rid" in {parameter["name"] for parameter in dataset_files_list["parameters"]}
...
def test_hidden_commands_are_excluded_recursively() -> None:
    ...
    root = click.Group(
        commands={
            "visible": click.Command("visible", help="Visible command", params=[json_option()]),
            "hidden": click.Command("hidden", hidden=True, params=[json_option()]),
            "nested": click.Group(
                commands={
                    "visible": click.Command("visible", help="Nested command", params=[json_option()]),
                    "hidden": click.Command("hidden", hidden=True, params=[json_option()]),
                }
            ),
        }
    ...
    root = click.Group(
        commands={
            "status": click.Group(
                ...
                params=[json_option()],
                commands={
                    "detail": click.Command("detail", help="Show details", params=[json_option()]),
                },
            )
        }
    )
