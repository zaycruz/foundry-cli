...
def test_drift_check_passes_against_fixture_manifest(tmp_path, capsys) -> None:
    reference_dir = _write_reference_dir(
        ...
        """
```bash
pfoundry dataset list
pfoundry ontology object-type-list ONTOLOGY_RID
```
""",
    )
...
def test_drift_check_accepts_typed_manifest_paths(tmp_path, capsys) -> None:
    ...
    reference_dir = _write_reference_dir(
        tmp_path, "`pfoundry dataset list`\n`pfoundry ontology object-type-list RID`\n"
    )
...
def test_drift_check_missing_manifest_is_actionable(tmp_path, capsys) -> None:
    reference_dir = _write_reference_dir(tmp_path, "`pfoundry dataset list`\n")
    ...
    assert "uv run pfoundry agent-manifest" in captured.err
