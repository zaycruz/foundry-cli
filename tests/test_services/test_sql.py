...
class TestSqlService:
    ...
    def test_execute_query_immediate_failure(self, service, mock_client):
        ...
        # Execute and assert
        with pytest.raises(RuntimeError, match="Query failed: Syntax error in SQL query"):
    ...
    def test_execute_query_with_waiting(self, service, mock_client):
        ...
        with patch('time.sleep'):  # Mock sleep to speed up test
            result = service.execute_query("SELECT COUNT(*) FROM large_table", timeout=1)
    ...
    def test_format_query_results_binary(self, service):
        ...
        results_bytes = b'\xff\xfe\xfd\x00\x01\x02'  # Binary data that can't be decoded as UTF-8
