...
def test_get_dataset_success(mock_dataset_service, sample_dataset):
    ...
    datasets_service.get_dataset.assert_called_once_with("ri.foundry.main.dataset.test-dataset")
...
def test_delete_dataset_success(mock_dataset_service):
    ...
    datasets_service.delete_dataset.assert_called_once_with("ri.foundry.main.dataset.test-dataset")
...
def test_format_dataset_info_minimal():
    ...
    for attr in ['name', 'description', 'created_time', 'created_by', 'last_modified', 'size_bytes', 'schema_id', 'parent_folder_rid']:
