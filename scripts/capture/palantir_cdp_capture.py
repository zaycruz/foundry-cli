...
async def write_record(rec):
    async with out_lock:
        rec["capturedAt"] = datetime.datetime.now(datetime.timezone.utc).isoformat()
    ...
    while True:
        ...
        try:
            async with websockets.connect(ws_url, max_size=100_000_000) as ws:
                ...
                print(
                    "[ready] capturing — browser restarts will be re-attached",
                    flush=True,
                )
                await recv_task
        except Exception as e:
...
def main():
    ...
    parser.add_argument("--outdir", default=os.path.expanduser("~/.palantir-capture"))
