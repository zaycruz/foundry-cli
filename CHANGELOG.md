...
- `ai-fde run` CLI extension tools (TICKET-009): five pfoundry-native synthetic tools added BEYOND the captured 72-tool catalog — clearly marked as CLI extensions (`cliExtension` in the registry, a `pfoundry_` name prefix, and a `<cliNotes>` instructions line) — because live runs proved the captured catalog alone cannot answer "show me the most recent runs of the <name> pipeline" (the catalog has no name-search tool by Palantir's design, and no build-history/dataset-transaction tool at all). All are read-risk, always exposed (not mode-gated), flow through the same approval/write-back/report machinery, and wrap already-verified pfoundry surfaces (public SDK / existing service contracts, no reverse engineering): `pfoundry_search_resources` (title→RID via `SearchService.search`, the same pinned GraphQL query as `pfoundry search`), `pfoundry_search_object_types` (object-type discovery by case-insensitive api_name/display_name substring via `OntologyService.list_ontologies` + `ObjectTypeService.list_object_types` — object types are not Compass resources, so title search cannot see them; per-ontology failures are recorded, not raised), `pfoundry_search_builds` (recent builds via `OrchestrationService.search_builds`/`get_build_jobs` around SDK `Build.search`/`Build.jobs`; `datasetRid` filtering is client-side over job outputs because the SDK filter vocabulary has no dataset member — the result reports the scan count), `pfoundry_get_dataset_transactions` (`DatasetService.get_transactions`/`get_branch_transactions` with client-side limit), and `pfoundry_get_resource` (`ResourceService.get_resource`). Executor errors return as tool output instead of crashing the loop. New module `services/ai_fde_extension_tools.py`.
- `ai-fde run` full tool enablement (TICKET-009): the complete 72-tool AI FDE catalog is now registered verbatim in `services/ai_fde_tool_specs.py` (extracted from the richest captured request; a pinned SHA-256 over the canonical JSON regression-guards the extraction). A mined mode→tool-set system replaces the flat MVP registry: the default is the captured 8-tool base set (no mode selected), `change_mode` now actually swaps the exposed tool set for subsequent LLM requests (`functionsEditing` → the captured 51-tool set; the `executeAction` capability adds `execute_action` + `await_automation_execution`, matching the captured transition), modes never selected in the capture keep the current set with an explicit note, and a new `--all-tools` flag exposes all 72 specs from turn 1 (fail-closed executors stay fail-closed with a typed `UnverifiedContract` tool result). Executor coverage grew from 13 to 25 live tools: new live reads `load_object_types`/`load_action_types` (captured identifier-form and rid-form `bulkLoadEntities` bodies), `load_link_types` (pinned `LinkTypeMainQuery`), `get_link_types_for_object_type`/`get_action_types_for_object_type` (pinned `Associated*MainQuery` GraphQL reads + captured detail loads; first page only, `_truncated` marker on uncaptured pagination), `load_object_sets` (`GET /object-set-service/api/objectSets/{rid}`), `load_functions` (`GET /function-registry/api/functions/{rid}/specs/{version}` with latest-version resolution), `load_code_repo` (captured stemma resolve + root listing + AGENTS.md triple, `master` ref), `load_pull_request` (delegates to the contract-verified `RepositoryService`), `container_git_status` (captured container chain: `GetContainersForRepository` → deployment create → status poll → `git/status`; boots a container like the UI does), plus approval-gated `container_execute_terminal_command` (captured `{command, directory?}` → `{exitCode, stdout, stderr}` with the captured "Terminal command executed" serialization; the UI's auto-approval classifier was not captured, so every command gates). The context budget rises to ~280k estimated tokens (the captured instructions state a 1,050,000-token window with a 300,000 recommended limit) so the full catalog fits. The schedules family, `ci_checks`, the logic family, documentation tools, and 41 other specs stay fail-closed — no endpoint for them exists in the 8,143-request capture (evidence per tool in the `ai_fde_loop.py` docstring); fail-closed write tools never trigger the approval prompt. UX: default stdout is now the final assistant text rendered as full-width Markdown (the run report table mangled long answers; `-f json` still yields the structured report), the run help documents that AI FDE has no resource-search tool even in the full catalog (name RIDs explicitly, as the UI's @-mentions are unavailable), and a clarification loop guard injects a best-effort directive with a remaining-turns countdown after 2 consecutive `request_clarification_from_user` calls with no intervening tool use (interactive TTY prompting unchanged).
- `ai-fde run "<instruction>"` — the CLI-side AI FDE agent loop (TICKET-009). Reimplements the Foundry UI's client-side loop: creates or resumes a thread (`--thread`), sends the instruction, calls the LLM (`PUT /language-model-service/api/llm/v3/completion/{model}/streamCompletionChunk`, default `GPT_5_6_SOL`, overridable via `--model`), executes returned tool calls, writes `tool-usage`/`assistant-message` items back into the thread, and repeats until completion or `--max-turns` (default 25). LLM `callId`s are rewritten to per-call UUIDs matching the captured UI behavior; tool results are serialized in the captured `<context-item>` pattern; context is budgeted (~60k estimated tokens) with captured hidden-item truncation. The captured request contract is recorded in `services/ai_fde_loop.py`, including the verified finding that the attribution `user` is the bearer token itself and that the chunk response is a single JSON array of events. Registered tools (specs captured verbatim in `services/ai_fde_tool_specs.py`): live reads `ontology_sql_query` (Arrow results decoded via pyarrow into the captured `<ontologySql>` serialization), `list_evaluation_runs`/`load_evaluation_runs`/`get_test_case_results`/`get_evaluation_suite_definition` (via `EvalsService`), `load_skill`; live state tools `change_mode`/`enable_capabilities`/`disable_capabilities`/`manage_context`/`request_clarification_from_user`; approval-gated writes `execute_action` (plan-first: `actions/validate` then `actionsV2`) and `run_evaluation_suite` (v2 config + latest-version resolution then the captured run contract); `load_documentation`/`load_documentation_bundles` are registered spec-only and fail closed with a typed `UnverifiedContract` error (no documentation page-load endpoint exists in the 1896-request capture). Write tools require operator approval: interactive prompt by default, `--yes` auto-approves, and `--agent`/non-interactive runs without `--yes` decline writes while read tools still execute. `--tools` restricts the offered tool subset; progress streams to stderr and the run report (`threadId`, `status`, `turns`, `toolsCalled`, `toolCalls`, `finalText`, `usage`) goes to stdout.
- `ai-fde` command vertical (AI FDE thread primitives): `ai-fde threads list|get|items|create|send`, `ai-fde threads metadata get|update`, and `ai-fde settings get|update`, backed by the internal `ai-fde` Conjure service plus pinned GraphQL gateway operations. All contracts were captured from the live AI FDE UI via Chrome DevTools Protocol (a live Foundry deployment, all HTTP 200); contracts recorded in the `services/ai_fde.py` docstring. Key finding: the AI FDE agent loop runs client-side in the Foundry UI — these commands manage the thread document, not the LLM loop. `threads send` appends a captured-shape `user-message` context item via `PUT /ai-fde/api/threads/{id}/update` and prints the resulting `threadVersion`; the `itemsToWrite` delta behavior is partially-derived (only a full-thread write was captured) and marked as such. `threads create` runs the captured `CreateThreadMutation` through a new scoped GraphQL mutation exception: a `VERIFIED_GRAPHQL_MUTATION_NAMES` registry in `foundry_internal_client` plus a per-call `allow_mutation_names` opt-in — both required, general mutation ban unchanged (TICKET-009).
- `evals` command vertical (AIP Evals): `evals suite list|get|evaluators|auto-metrics|suggested-scope` and `evals run list|summary|test-cases|trigger`, backed by the internal `foundry-evals` Conjure service. All 13 endpoint contracts were captured from the live AIP Evals UI via Chrome DevTools Protocol (a live Foundry deployment, all HTTP 200); contracts recorded in the `services/evals.py` docstring. `evals run trigger` is plan-first: default prints the resolved run body plus the read-only `suggestedExecutionScope`, `--apply` issues the run and prints `{buildRid, jobRid, executionId}`. The suite config-write endpoint was never observed and is deliberately not implemented (TICKET-007).
- `ontology action-apply-with-overrides` — wraps the SDK `Action.apply_with_overrides` endpoint (POST `applyWithOverrides`) for UniqueIdentifier and CurrentTime generated action parameters, plus a service-level `apply_batch_with_overrides` (20-request cap kept). Plan-first: default prints the VALIDATE_ONLY validation result, `--apply` executes.
- `ontology object-upsert` — create-or-update one object instance by primary key via an action (object instances have no direct write endpoint). Default prints the plan: create-vs-update decision from reading the object by primary key, resolved action parameters (primary key pinned, conflicts rejected), and the VALIDATE_ONLY validation result. `--apply` executes (with overrides when `--overrides` is given) and reads the object back; a failed or contradicting read-back raises loudly.
- `ontology object-upsert-batch` — the batch counterpart: a JSON rows file (`-` reads stdin), chunked into groups of at most 20 per applyBatch request, same plan/`--apply` split, per-row read-back report; any row that cannot be verified fails the command.
