...
pltr media-sets get-by-path "$MEDIA_SET" "/images/photo.jpg" \
  --profile "$PROFILE" --format json

# Capture the RID for scripting (JSON keys: path, rid)
ITEM_RID=$(pltr media-sets get-by-path "$MEDIA_SET" "/images/photo.jpg" \
  --profile "$PROFILE" --format json | jq -r '.rid')

# Get detailed info about the item
pltr media-sets get "$MEDIA_SET" "$ITEM_RID" \
  --profile "$PROFILE" --format json
...
```bash
pltr media-sets reference "$MEDIA_SET" "$ITEM_RID" \
  --profile "$PROFILE" --format json
```
...
1. **Create**: `pltr media-sets create` opens a transaction and prints the transaction ID.
2. **Upload**: `pltr media-sets upload` stages one file per call into the transaction. Staged files are not visible to other consumers.
3. **Commit**: `pltr media-sets commit` makes all staged uploads available atomically. Prompts for confirmation unless `--yes` is passed.
4. **Abort**: `pltr media-sets abort` discards the transaction and deletes anything staged in it. Prompts for confirmation unless `--yes` is passed.
...
- `pltr media-sets create` has no `--format` option. It prints `Transaction ID: <id>` as a status line; capture it by parsing the output.
- Status lines go to stderr when stdout is piped, so merge the streams when capturing:

```bash
TX=$(pltr media-sets create "$MEDIA_SET" --profile "$PROFILE" 2>&1 \
...
# 2. Stage the file
pltr media-sets upload "$MEDIA_SET" ./report.pdf "/documents/report.pdf" "$TX" \
  --profile "$PROFILE"

# 3. Commit (explicit confirmation)
pltr media-sets commit "$MEDIA_SET" "$TX" --yes --profile "$PROFILE"
...
abort_tx() {
  ...
  pltr media-sets abort "$MEDIA_SET" "$TX" --yes --profile "$PROFILE"
  exit 1
}

# 1. Create the transaction
TX=$(pltr media-sets create "$MEDIA_SET" --branch "$BRANCH" --profile "$PROFILE" 2>&1 \
  | sed -n 's/.*Transaction ID: //p')
if [ -z "$TX" ]; then
...
for file in "$LOCAL_DIR"/*; do
  ...
  if pltr media-sets upload "$MEDIA_SET" "$file" "$REMOTE_PREFIX/$filename" "$TX" \
...
pltr media-sets download "$MEDIA_SET" "$ITEM_RID" ./photo.jpg \
  --profile "$PROFILE"

# Download the original rendition as uploaded
pltr media-sets download "$MEDIA_SET" "$ITEM_RID" ./photo-original.jpg \
...
ITEM_RID=$(pltr media-sets get-by-path "$MEDIA_SET" "/images/photo.jpg" \
  --profile "$PROFILE" --format json | jq -r '.rid')
pltr media-sets download "$MEDIA_SET" "$ITEM_RID" ./photo.jpg --profile "$PROFILE"
```
...
These operations are not exposed by `pltr media-sets`; do not guess at them:
...
- `pltr media-sets create` emits no machine-readable format; scripts must parse the `Transaction ID:` status line.
...
4. **Verify auth first**: run `pltr verify` at script start
