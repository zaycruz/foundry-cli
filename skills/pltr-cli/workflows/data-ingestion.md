...
```bash
pltr verify --profile "$PROFILE"
pltr dataset get ri.foundry.main.dataset.abc123 --profile "$PROFILE"
pltr dataset files list ri.foundry.main.dataset.abc123 --branch master
```
...
```bash
pltr dataset files upload "./export_001.csv" ri.foundry.main.dataset.abc123 \
...
- `pltr dataset transactions list` is dataset-wide; the pinned SDK transaction endpoint has no branch filter.
- After commit, verify with `pltr dataset files list` or `pltr dataset stats DATASET_RID --branch master`.
...
pltr connectivity import list-file --connection ri.conn.main.connection.12345
pltr connectivity import list-table --connection ri.conn.main.connection.12345
...
pltr streams stream get ri.foundry.main.dataset.abc123 --branch master --format json

# 3. Publish records — prefer batches over per-record calls
pltr streams stream publish-batch ri.foundry.main.dataset.abc123 \
  --branch master \
  --records '[{"event_id": "evt-001", "timestamp": 1735689600, "data": "payload"}]'

# Single record when needed
pltr streams stream publish ri.foundry.main.dataset.abc123 \
  --branch master --record @record.json
```

For volume, size partitions up front: each partition handles roughly 5 MB/s (`--partitions N --type HIGH_THROUGHPUT` on `streams dataset create`). To add a stream on another branch of an existing dataset, use `pltr streams stream create DATASET_RID --branch BRANCH --schema SCHEMA`.
...
for f in "${FILES[@]}"; do
  echo "Uploading $f"
  pltr dataset files upload "$f" "$DATASET_RID" \
...
pltr dataset files list "$DATASET_RID" --branch "$BRANCH" --profile "$PROFILE"
