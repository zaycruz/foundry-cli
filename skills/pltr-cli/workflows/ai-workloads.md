...
pltr language-models list --profile "$PROFILE"
...
pltr functions query get monthlyReport --format json
# or by RID
...
pltr functions query execute monthlyReport \
  --parameters '{
...
pltr functions query execute monthlyReport --version 1.0.0 --parameters '{}'
```
...
pltr aip-agents get $AGENT --format json
pltr aip-agents get $AGENT --version 1.5 --format json

# Version history (descending, most recent first)
pltr aip-agents versions list $AGENT --all --format csv --output versions.csv

# Conversation sessions
pltr aip-agents sessions list $AGENT --all --format json --output sessions.json
pltr aip-agents sessions get $AGENT ri.foundry.main.session.xyz789 --format json
```

Compare two agent versions:

```bash
pltr aip-agents get $AGENT --version 1.0 --format json --output v1.json
pltr aip-agents get $AGENT --version 2.0 --format json --output v2.json
...
# Model metadata
pltr models model get $MODEL --format json

# Version history (token pagination)
pltr models version list $MODEL --page-size 50 --format json --output versions.json
pltr models version list $MODEL --page-size 50 --page-token <token-from-previous-response>

# Specific version details
pltr models version get $MODEL v1.0.0 --format json --output v1.json
```
