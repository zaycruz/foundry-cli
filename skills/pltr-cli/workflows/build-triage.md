...
- There is no dedicated `builds rerun` command. A rerun is `pltr orchestration builds create` with the same target JSON.
...
- `pltr compute logs` covers compute-module jobs only. There is no log command for Spark or other job types; document that as a gap when the failing job is not a compute-module job.
- `pltr data-health report get` requires both a check RID and a report RID. There is no command to list reports for a check; if report RIDs are not visible in the check details, report the gap.
...
```bash
pltr orchestration builds search --profile "$PROFILE" --format json --output builds.json
```
...
pltr orchestration jobs get-batch "$JOB_RIDS" --profile "$PROFILE" --format json --output job-details.json
```

Use `pltr orchestration jobs get ri.orchestration.main.job.def456 --format json` for a single job. `get-batch` accepts a comma-separated list, max 500. Record each failing job's type, status, duration, and output dataset RID.
...
```bash
pltr compute logs ri.orchestration.main.job.def456 \
...
```bash
pltr sql execute "SELECT COUNT(*) AS row_count FROM my_output_dataset" \
  --profile "$PROFILE"

pltr sql execute "SELECT * FROM my_output_dataset LIMIT 10" \
  --profile "$PROFILE" --format json --output sample.json

pltr sql execute "SELECT MAX(updated_at) AS latest FROM my_output_dataset" \
  --profile "$PROFILE"
```

Use `pltr sql submit` + `pltr sql wait` for heavier checks. Zero rows after a "successful" build, stale `MAX` timestamps, or nulls in required columns all point remediation at the pipeline logic, not at a retry.
...
pltr orchestration builds create \
...
```bash
pltr orchestration builds get ri.orchestration.main.build.new456 \
  --profile "$PROFILE" --format json

pltr orchestration schedules runs ri.orchestration.main.schedule.ghi789 \
