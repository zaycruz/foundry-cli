...
- The CLI exposes no group membership read or mutation commands (`pltr admin group` offers only `list`, `get`, `search`, `create`, `delete`, `batch-get`). Membership inspection, add, and remove are performed in the Foundry platform UI; record them in the plan and verification as out-of-band steps.
- `pltr resource-role grant` has no `--confirm` flag and applies immediately. Treat the read phases below as its only review gate, and never run it before the plan is accepted.
- `pltr audit` returns raw log files for an organization; it does not filter by user, action, or resource. Filtering is a local post-processing step.
...
```bash
pltr admin user current --profile "$PROFILE" --format json
pltr admin org get "$ORGANIZATION_ID" --profile "$PROFILE" --format json
```
...
```bash
# Full user inventory
pltr admin user list --profile "$PROFILE" --page-size 50 \
  --format json --output ./audit_users.json

# Targeted lookup
pltr admin user search "$QUERY" --profile "$PROFILE" --page-size 20 --format json
pltr admin user get "$USER_ID" --profile "$PROFILE" --format json
pltr admin user markings "$USER_ID" --profile "$PROFILE" --format json

# Batch resolution (max 500 IDs)
pltr admin user batch-get "$USER_ID_1" "$USER_ID_2" --profile "$PROFILE"

# Group inventory
pltr admin group list --profile "$PROFILE" --format json --output ./audit_groups.json
pltr admin group search "$QUERY" --profile "$PROFILE" --format json
pltr admin group get "$GROUP_ID" --profile "$PROFILE" --format json
```

Extract and record the principal UUID for each user or group that a later mutation will target. `pltr resource-role grant` and `revoke` require the principal UUID; an email address or display name is not accepted.
...
pltr resource-role list "$RESOURCE" --profile "$PROFILE" \
  --format json --output ./audit_resource_roles.json

# Narrow by principal type when the grant list is large
pltr resource-role list "$RESOURCE" --principal-type User --profile "$PROFILE"
pltr resource-role list "$RESOURCE" --principal-type Group --profile "$PROFILE"

# Resolve role IDs discovered in the grant list (max 500 per call)
pltr admin role get "$ROLE_ID" --profile "$PROFILE" --format json
pltr admin role batch-get "$ROLE_ID_1" "$ROLE_ID_2" --profile "$PROFILE"

# Enumerate roles grantable in the organization
pltr admin org available-roles "$ORGANIZATION_RID" --profile "$PROFILE" \
  --page-size 50 --format json

# Review markings and access requirements gating the resource
pltr resource list-markings "$RESOURCE" --profile "$PROFILE" --format json
pltr resource access-requirements "$RESOURCE" --profile "$PROFILE" --format json
```
...
```bash
pltr admin user get "$USER_ID" --profile "$PROFILE" --format json
pltr resource-role list "$RESOURCE" --profile "$PROFILE" --format json \
