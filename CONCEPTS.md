...
### Inconclusive
A coverage status meaning "could not prove absence" — distinct from a verified empty result. An empty, truncated, permission-denied, or expired-token response from an internal API becomes Inconclusive, never `covered-empty`. It exists so a silent API failure can never read as "no impact."
